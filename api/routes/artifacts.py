...
# Errors that mean "the AWS-backed scanner pipeline is not reachable/configured".
...
async def submit_scan(body: dict):
    try:
        result = submit_scan_handler({"body": body}, None)
...
async def get_artifact(artifact_id: str):
    try:
        result = artifact_status_handler({"artifact_id": artifact_id}, None)
    except _AWS_UNAVAILABLE_ERRORS as exc:
        return _unavailable(f"AWS scanner backend unavailable: {type(exc).__name__}: {exc}")
    return _relay(result)
